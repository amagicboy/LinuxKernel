			    ...
			    struct perf_event_context *ctx)
{
	...
	if (event->pending_sigtrap) {
		...
		if (state != PERF_EVENT_STATE_OFF && !event->pending_work) {
				...
				enum event_type_t event_type)
{
	...
	if (event_type & EVENT_PINNED) {
		list_for_each_entry_safe(event, tmp, &pmu_ctx->pinned_active,
					 active_list)
			group_sched_out(event, ctx);
	}

	if (event_type & EVENT_FLEXIBLE) {
		list_for_each_entry_safe(event, tmp, &pmu_ctx->flexible_active,
					 active_list)
			group_sched_out(event, ctx);
		/*
...
static void ctx_pinned_sched_in(struct perf_event_context *ctx, struct pmu *pmu)
{
	...
	if (pmu) {
		visit_groups_merge(ctx, &ctx->pinned_groups, smp_processor_id(),
				   pmu, merge_sched_in, &can_add_hw);
	} else {
		list_for_each_entry(pmu_ctx, &ctx->pmu_ctx_list,
				    pmu_ctx_entry) {
			can_add_hw = 1;
				  ...
				  struct pmu *pmu)
{
	...
	if (pmu) {
		visit_groups_merge(ctx, &ctx->flexible_groups,
				   smp_processor_id(), pmu, merge_sched_in,
				   &can_add_hw);
	} else {
		list_for_each_entry(pmu_ctx, &ctx->pmu_ctx_list,
				    pmu_ctx_entry) {
...
got_name:
	...
	if (atomic_read(&nr_build_id_events))
		build_id_parse(vma, mmap_event->build_id,
